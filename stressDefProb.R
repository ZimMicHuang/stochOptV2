runTest_defProb = function(
  ...
  for (i in 1:S) {
    ...
    CF_stoch[i,] = t(stoch_mod$soln)%*%Cf/((1+RF/12)^(1:TF)) # discount -> PV's
    CF_base[i,] = t(base_mod$soln)%*%Cf/((1+RF/12)^(1:TF)) 
  }
  ...
  l_NPV = as.numeric(l_df %*% (1/((1+RF/12)^(1:TF))))
